...
select t1.id,
			 ...
			 sysdate()
	from t_subject t1
 where t1.platform_id = 17
	 and not exists(select 1 from temp_subject_yqh t where t.link = t1.link)
;
...
/*2014-12-25更新*/
#历史表已经存在100%的标的无需进数
delete from temp_subject_yqh 
 where round(substr(replace(process, '份', ''), 1, instr(replace(process, '份', ''), '/') - 1) /
						(substr(replace(process, '份', ''), instr(replace(process, '份', ''), '/') + 1) +
						 substr(replace(process, '份', ''), 1, instr(replace(process, '份', ''), '/') - 1))
						* 100, 2) = 100 /*updated by daniel on 2014-12-31*/
	 and exists(select 1 from t_subject_his t1 where t1.link = temp_subject_yqh.link )
;

#更新change_his表中进度大于抓取标的进度的数据
delete from t_subject_change_his /*updated by daniel on 2015-1-5*/
 where exists(select 1 from temp_subject_yqh t, t_subject t2
											where t2.link = t.link
											  and t2.id = t_subject_change_his.subject_id
												and t_subject_change_his.process_after > round(substr(replace(t.process, '份', ''), 1, instr(replace(t.process, '份', ''), '/') - 1) /
						(substr(replace(t.process, '份', ''), instr(replace(t.process, '份', ''), '/') + 1) +
						 substr(replace(t.process, '份', ''), 1, instr(replace(t.process, '份', ''), '/') - 1))
						* 100, 2) ) #注意此处process逻辑要修改
;
...
select t1.id,
			 ...
			 sysdate()
	from temp_subject_yqh t
 inner join t_subject t1 on t1.link = t.link and t1.process <> round(substr(replace(t.process, '份', ''), 1, instr(replace(t.process, '份', ''), '/') - 1) /
						(substr(replace(t.process, '份', ''), instr(replace(t.process, '份', ''), '/') + 1) +
						 substr(replace(t.process, '份', ''), 1, instr(replace(t.process, '份', ''), '/') - 1))
						* 100, 2)  #注意此处process逻辑要修改
;
...
select t1.id,
			 ...
			 sysdate()
	from temp_subject_yqh t
 inner join t_subject_his t1 on t1.link = t.link and t1.process <> round(substr(replace(t.process, '份', ''), 1, instr(replace(t.process, '份', ''), '/') - 1) /
						(substr(replace(t.process, '份', ''), instr(replace(t.process, '份', ''), '/') + 1) +
						 substr(replace(t.process, '份', ''), 1, instr(replace(t.process, '份', ''), '/') - 1))
						* 100, 2)  #注意此处process逻辑要修改
;
...
from (select t1.id,
			 ...
			 case when t.name like '%车%' then '车贷'
						when t.name like '%房%' and t.name like '%抵%' then '房贷'
						when t.name like '%企业%' or t.name like '%集团%' then '企业贷'
						else '个人信用贷' end type,
			 ...
			 t.repay_type,
			 t.publish_time,
			 case when round(substr(replace(t.process, '份', ''), instr(replace(t.process, '份', ''), '/') + 1)
											/ substr(replace(t.process, '份', ''), 1, instr(replace(t.process, '份', ''), '/') - 1)
											* 100, 2) = 100 then sysdate() else null end end_time,
			 ...
			 trim(t.description) description,
			 round(substr(replace(t.process, '份', ''), 1, instr(replace(t.process, '份', ''), '/') - 1) /
						(substr(replace(t.process, '份', ''), instr(replace(t.process, '份', ''), '/') + 1) +
						 substr(replace(t.process, '份', ''), 1, instr(replace(t.process, '份', ''), '/') - 1))
						* 100, 2) process
	from temp_subject_yqh t
...
delete from t_subject_his
 where exists(select 1 from temp_subject_yqh t1 where t1.link = t_subject_his.link)
;
/*2014-12-25更新结束*/
