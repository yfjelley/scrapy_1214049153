...
#删除重复数据
delete from temp_subject_ppd
 where id not in (select id from (select max(id) as id from temp_subject_ppd  group by link) a);
...
select t1.id,
			 ...
			 sysdate()
	from t_subject t1
 where t1.platform_id = 13
	 and not exists(select 1 from temp_subject_ppd t where t.link = t1.link)
;
...
delete from temp_subject_ppd 
 where process = 100 
	 and exists(select 1 from t_subject_his t1 where t1.link = temp_subject_ppd.link )
;

#更新change_his表中进度大于抓取标的进度的数据
update t_subject_change_his t1
	 set t1.valid_status = 0
 where exists(select 1 from temp_subject_ppd t, t_subject t2
...
select t1.id,
			 ...
			 t.link,
			 t.name,
			 replace(replace(t.amount, '借款总额： ¥', ''), ',', '') amount,
			 case t.min_amount when '' then null else trim(replace(replace(t.min_amount, ',', ''), '￥', '')) end min_amount,
			 t.income_rate,
			 case when t.term like '%月%' then trim(replace(t.term, '个月', '')) else t.term end as term,
...
delete from t_subject_his
 where exists(select 1 from temp_subject_ppd t1 where t1.link = t_subject_his.link and t1.process <> 100)
;
/*2014-12-25更新结束*/
