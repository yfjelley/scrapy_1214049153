 ...
 where exists(select 1 from temp_subject_yqh t, t_subject t2
											where t2.link = t.link
											  and t2.id = t1.subject_id
												and t1.process_after > round(substr(replace(t.process, '份', ''), 1, instr(replace(t.process, '份', ''), '/') - 1) /
...
delete from t_subject_his
 where exists(select 1 from temp_subject_yqh t1 where t1.link = t_subject_his.link and t1.process <> 100)
;
/*2014-12-25更新结束*/
