...
select t1.id,
			 ...
			 sysdate()
	from t_subject t1
 where t1.platform_id = 6
	 and not exists(select 1 from temp_subject_yl t where t.link = t1.link)
;
...
/*2014-12-25更新*/
#历史表已经存在100%的标的无需进数
delete from temp_subject_yl 
 where process = 100 
	 and exists(select 1 from t_subject_his t1 where t1.link = temp_subject_yl.link )
;

#更新change_his表中进度大于抓取标的进度的数据
update t_subject_change_his t1
	 set t1.valid_status = 0
 where exists(select 1 from temp_subject_yl t, t_subject t2
											where t2.link = t.link
											  and t2.id = t1.subject_id
												and t1.process_after > t.process ) #注意此处process逻辑要修改
;
...
delete from t_subject_his
 where exists(select 1 from temp_subject_yl t1 where t1.link = t_subject_his.link and t1.process <> 100)
;
/*2014-12-25更新结束*/
