...
select t1.id,
			 ...
			 sysdate()
	from t_subject t1
 where t1.platform_id = 7
	 and not exists(select 1 from temp_subject_el t where t.link = t1.link)
;
...
/*2014-12-25更新*/
#历史表已经存在100%的标的无需进数
delete from temp_subject_el 
 where replace(process, '%', '') = 100 /*updated by daniel on 2014-12-31*/
	 and exists(select 1 from t_subject_his t1 where t1.link = temp_subject_el.link )
;

#更新change_his表中进度大于抓取标的进度的数据
delete from t_subject_change_his /*updated by daniel on 2015-1-5*/
 where exists(select 1 from temp_subject_el t, t_subject t2
...
from (select t1.id,
			 ...
			 replace(t.income_rate, '%/年', '') income_rate,
...
delete from t_subject_his
 where exists(select 1 from temp_subject_el t1 where t1.link = t_subject_his.link)
;
/*2014-12-25更新结束*/
