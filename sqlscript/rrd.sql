...
select t1.id,
			 ...
			 sysdate()
	from t_subject t1
 where t1.platform_id = 2
	 and not exists(select 1 from temp_subject_rrd t where t.link = t1.link)
;
...
from (select t1.id,
			 ...
			 case when t.name like '%车%' and t.name like '%抵%' then '车贷'
			 ...
			 t.reward,
			 trim(t.protect_mode) protect_mode,
			 t.description,
...
delete from t_subject_his
 where exists(select 1 from temp_subject_rrd t1 where t1.link = t_subject_his.link)
;
/*2014-12-25更新结束*/
