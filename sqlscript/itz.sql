...
select t1.id,
			 ...
			 sysdate()
	from t_subject t1
 where t1.platform_id = 12
	 and not exists(select 1 from temp_subject_itz t where t.link = t1.link)
;
...
/*2014-12-25更新*/
#历史表已经存在100%的标的无需进数
delete from temp_subject_itz 
 where case when process = '' then 100 else replace(process, '%', '') end = 100 /*updated by daniel on 2014-12-31*/
	 and exists(select 1 from t_subject_his t1 where t1.link = temp_subject_itz.link )
;

#更新change_his表中进度大于抓取标的进度的数据
delete from t_subject_change_his /*updated by daniel on 2015-1-5*/
 where exists(select 1 from temp_subject_itz t, t_subject t2
											where t2.link = t.link
											  and t2.id = t_subject_change_his.subject_id
												and t_subject_change_his.process_after > case when t.process = '' then 100 else replace(t.process, '%', '') end ) #注意此处process逻辑要修改
;
...
from (select t1.id,
			 ...
			 t.link,
			 t.name,
			 case when t.amount like '%万%' then replace(t.amount, '万', '') * 10000 end amount,
			 t1.min_amount,
			 replace(t.income_rate, '%', '') income_rate,
			 replace(t.term, '个月', '') term,
...
delete from t_subject_his
 where exists(select 1 from temp_subject_itz t1 where t1.link = t_subject_his.link)
;
/*2014-12-25更新结束*/
