...
from (select t1.id,
			 ...
			 case when t.income_rate like '%年%' then replace(t.income_rate, '%/年', '')
						when t.income_rate like '%日%' then replace(t.income_rate, '%/日', '') * 365 else t.income_rate end income_rate,
			 case when t.term like '%月%' then replace(t.term, '个月', '')
						when t.term like '%天%' then replace(t.term, '天', '') / 30 else t.term end as term,
...
delete from t_subject_his
 where exists(select 1 from temp_subject_hlct t1 where t1.link = t_subject_his.link)
;
/*2014-12-25更新结束*/
