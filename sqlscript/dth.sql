...
delete from temp_subject_dth 
 where round(process,2) like '%100%' /*updated by daniel on 2014-12-31*/
 ...
 where exists(select 1 from temp_subject_dth t, t_subject t2
											where t2.link = t.link
											  and t2.id = t_subject_change_his.subject_id
												and t_subject_change_his.process_after > round(t.process,2) ) #注意此处process逻辑要修改
;
...
delete from t_subject_his
 where exists(select 1 from temp_subject_dth t1 where t1.link = t_subject_his.link)
;
/*2014-12-25更新结束*/
